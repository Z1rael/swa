...
. **Image service** splits image from metadata and sends them to **mq broker**
. **Image-Processing** service recceives image and tries to send it to **object storage**
. If successful receives **link to stored image** and sends it to **mq broker**
...
. Search request is sent to **Search Service** via Message Queue
